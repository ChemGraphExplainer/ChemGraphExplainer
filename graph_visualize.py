...
class RDKitMoleculeDrawer:
    ...
    def draw(self, ax):
        ...
        line1 = Line2D([start_x, mid_x], [start_y, mid_y], color=line_color, alpha=alpha, linewidth=4)
        line2 = Line2D([mid_x, end_x], [mid_y, end_y], color=line_color, alpha=alpha, linewidth=4)

        ax.add_line(line1)
        ax.add_line(line2)

        #ax.scatter(mid_x, mid_y, color='white', s=150)
        ...
        for i, atom_coord in enumerate(self.atom_coords):
            ...
            if i in [self.start_node_idx, self.end_node_idx, self.mid_node_idx]:
                circle = Circle((atom_coord[0], atom_coord[1]), radius=0.3, color=line_color, alpha=alpha)
                ax.add_patch(circle)
                ax.scatter(mid_x, mid_y, color='white', s=150)
            ax.text(atom_coord[0], atom_coord[1], atom_symbol, fontsize=13, ha='center', va='center')
           
       
        mol_xlim = (min(atom_coord[0] for atom_coord in self.atom_coords),
            max(atom_coord[0] for atom_coord in self.atom_coords))
        mol_ylim = (min(atom_coord[1] for atom_coord in self.atom_coords),
                    max(atom_coord[1] for atom_coord in self.atom_coords))
       
        # Ax'in boyutunu çizimin boyutundan daha büyük ayarla
        ax.set_xlim(mol_xlim[0] - 1, mol_xlim[1] + 1)  # Örneğin 0.5 birim daha büyük
        ax.set_ylim(mol_ylim[0] - 0.5, mol_ylim[1] + 3.2)
       
        ax.text(mol_xlim[0] - 0.8, mol_ylim[1]+3.0, f'Method: {self.method}', fontsize=10, ha='left', va='center')
        ax.text(mol_xlim[0] - 0.8, mol_ylim[1]+2.5, f'Smiles: {self.smiles}', fontsize=10, ha='left', va='center')
        ax.text(mol_xlim[0] - 0.8, mol_ylim[1]+2.0, f'Prediction: {self.prediction}', fontsize=10, ha='left', va='center')
        ax.text(mol_xlim[0] - 0.8, mol_ylim[1]+1.5, f'Sparsity: {self.sparsity:.4f}', fontsize=10, ha='left', va='center')
        ax.text(mol_xlim[0] - 0.8, mol_ylim[1]+1.0, f'Fidelity: {self.fidelity:.4f}', fontsize=10, ha='left', va='center')
        ax.text(mol_xlim[0] - 0.8, mol_ylim[1]+0.5, f'Fidelity_inv: {self.fidelity_inv:.4f}', fontsize=10, ha='left', va='center')
        ax.set_axis_off()
    ...
    def draw_bond_between_atoms(self, ax):
        ...
        line1 = Line2D([start_x, end_x], [start_y, end_y], color=line_color, alpha=alpha, linewidth=4)
        ...
        for i, atom_coord in enumerate(self.atom_coords):
            ...
            if i in [self.start_node_idx, self.end_node_idx]:#, self.mid_node_idx]:
                circle = Circle((atom_coord[0], atom_coord[1]), radius=0.3, color=line_color, alpha=alpha)
                ax.add_patch(circle)
                ax.scatter(start_x, start_y, color='white', s=150)
            ax.text(atom_coord[0], atom_coord[1], atom_symbol, fontsize=10, ha='center', va='center')
